document.addEventListener('DOMContentLoaded', () => {
    ...
    window.getListingPaymentInfo = function(listing, isRenewalTab = false) {
        ...
        if (globalMonthlyPrice === 0) {
            ...
            textDesc = `Total a pagar por renovación: $${formattedPrice} pesos`;
        } else {
            if (daysRemaining <= 5) {
                calculatedPrice = Math.ceil((pricePerDay * daysRemaining) + globalMonthlyPrice);
                const formattedPrice = calculatedPrice.toFixed(2);
                textDesc = `Total a pagar: $${formattedPrice} pesos (días restantes y próximo mes)`;
            } else {
                calculatedPrice = Math.ceil(pricePerDay * daysRemaining);
                const formattedPrice = calculatedPrice.toFixed(2);
                textDesc = `Total a pagar por el resto del mes: $${formattedPrice} pesos`;
            }
        }
        return {
            calculatedPrice,
        ...
        btnSavePrice.addEventListener('click', async () => {
            ...
            try {
                const res = await fetch('/api/settings', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ monthlyPrice: val })
                });
                const data = await res.json();
                if (data.success) {
                    globalMonthlyPrice = val;
                    showAlert('Costo mensual base actualizado correctamente.', 'Guardado', 'check_circle');
