document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadSettings() {
        try {
            ...
            if (data.success && data.settings) {
                ...
                if (costDisclaimer) {
                    costDisclaimer.innerHTML = `* Costo mensual: <strong>$${globalMonthlyPrice.toFixed(2)} MXN</strong> (Las renovaciones son los días 1ro de cada mes).`;
                }
    ...
    window.getListingPaymentInfo = function(listing, isRenewalTab = false) {
        ...
        const daysRemaining = daysInMonth - now.getDate() + 1; // incluyendo hoy
        const pricePerDay = globalMonthlyPrice / daysInMonth;
        ...
        if (globalMonthlyPrice === 0) {
            ...
            if (daysRemaining <= 5) {
                calculatedPrice = Math.ceil((pricePerDay * daysRemaining) + globalMonthlyPrice);
                const formattedPrice = calculatedPrice.toFixed(2);
                textDesc = `Total a pagar por ${daysRemaining + daysInMonth} días: $${formattedPrice} MXN`;
            } else {
                calculatedPrice = Math.ceil(pricePerDay * daysRemaining);
                const formattedPrice = calculatedPrice.toFixed(2);
                textDesc = `Total a pagar $${formattedPrice} MXN por ${daysRemaining} días`;
            }
        }
        return {
            calculatedPrice,
    ...
    async function populateSelects() {
        ...
        try {
            ...
            const data = await res.json();
            if (data.success) {
                window.activeLocations = data.locations;
            } else {
                        ...
                        if (stateName) {
                            ...
                            if (matchedState) {
                                ...
                                if (matchedCity) {
                                    ...
                                    // No se muestra la ciudad en el label, solo el estado
                                } else {
                                ...
                                // Actualizar el botón de ciudades con la ciudad detectada
                                if (window.updateCitiesBtn) window.updateCitiesBtn();
        ...
        // Auto-detectar ubicación una sola vez tras cargar la aplicación
        ...
        try {
            if (editingListingId) {
                ...
                if (globalMpEnabled) {
                    ...
                    if (priceText) priceText.textContent = `$${globalMonthlyPrice} MXN`;
                ...
                <div class="pending-row-header" onclick="togglePendingDetail(${listing.id})">
                    <div class="pending-row-left">
                        ...
                        <div class="pending-main-info">
                            <div class="pending-title">${listing.title}</div>
    ...
    function updateAdminRenewals() {
        ...
        // Determinar mes objetivo (el próximo mes si estamos a 5 días o menos del final, o el mes actual)
        const now = new Date();
        const year = now.getFullYear();
        const month = now.getMonth();
        const daysInMonth = new Date(year, month + 1, 0).getDate();
        const isRenewalPeriod = now.getDate() >= (daysInMonth - 5);
        
        let targetMonthStr = '';
        if (isRenewalPeriod) {
            // Requieren renovación para el próximo mes
            const nextDate = new Date(year, month + 1, 1);
            targetMonthStr = `${nextDate.getFullYear()}-${String(nextDate.getMonth() + 1).padStart(2, '0')}`;
        } else {
            // Fuera de periodo, la pestaña podría estar vacía o mostrar los que no renovaron para el mes actual
            targetMonthStr = `${year}-${String(month + 1).padStart(2, '0')}`;
        }

        let pendingRenewals = db.getAllListings().filter(l => {
            // Mostrar sólo autorizados que NO tengan el targetMonthStr en su lastRenewedMonth
            return l.status === 'autorizado' && l.lastRenewedMonth !== targetMonthStr;
        });
        ...
        if (pendingRenewals.length === 0) {
            list.innerHTML = query 
                ? '<p style="color:var(--text-muted); text-align:center; padding: 20px;">No se encontraron resultados para la búsqueda.</p>'
                : (isRenewalPeriod ? '<p style="color:var(--text-muted); text-align:center; padding: 20px;">No hay renovaciones pendientes para el próximo mes. ¡Todos al corriente!</p>' : '<p style="color:var(--text-muted); text-align:center; padding: 20px;">No estamos en periodo de renovación masiva. Las renovaciones inician 5 días antes de fin de mes.</p>');
            return;
        }
            ...
            <div class="pending-approval-card ${isExpanded ? 'expanded' : ''}" id="pending-card-${listing.id}">
                <div class="pending-row-header" onclick="togglePendingDetail(${listing.id})">
                    ...
                    <div class="pending-row-right">
                        ...
                        <button class="success-btn" onclick="event.stopPropagation(); renewListingAdmin(${listing.id}, '${targetMonthStr}')" title="Marcar como pagado/renovado" style="padding: 6px 12px; display:flex; align-items:center; gap:4px;">
    ...
    window.renewListingAdmin = function(id, monthStr, isConfirmed = false) {
        if (!isConfirmed) {
            pendingRenewActionTargetId = id;
            pendingRenewActionMonthStr = monthStr;
        ...
        if(listing) {
            listing.lastRenewedMonth = monthStr;
            localStorage.setItem(db.listingsKey, JSON.stringify(listings));
            
            fetch(`${db.apiBaseUrl}/listings/${id}/renew`, { 
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ month: monthStr })
            }).catch(err => console.error('Error renovando en el servidor', err));
            
            const modal = document.getElementById('renew-confirm-modal');
            if (modal) modal.classList.remove('active');

            updateAdminRenewals();
            showAlert('La publicación ha sido renovada exitosamente.', 'Renovación Exitosa', 'autorenew');
        }
    };
    ...
    window.approveListing = function(id, isConfirmed = false) {
        ...
        if(listing) {
            ...
            fetch(`${db.apiBaseUrl}/listings/${id}/approve`, { method: 'PUT' })
                .catch(err => console.error('Error aprobando en el servidor', err));
    ...
    window.deleteListingAdmin = async function(id, isConfirmed = false) {
        ...
        try {
            ...
            const data = await res.json();
            if (!data.success) {
                showAlert('Error del servidor: ' + data.error, 'Error al borrar', 'error');
                return;
            }
        } catch(e) {
            console.error('Error al borrar en el servidor', e);
            showAlert('Hubo un error de conexión al borrar.', 'Error', 'error');
            return;
        }
    ...
    if (btnSaveAdminEdit) {
        btnSaveAdminEdit.onclick = async (e) => {
            ...
            if (listingIndex > -1) {
                ...
                try {
                    await fetch(`${db.apiBaseUrl}/listings/${adminEditTargetId}`, {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify(listings[listingIndex])
                    });
                } catch(e) {
                    console.error('Error actualizando en servidor', e);
                }
    ...
    if (btnRenewYes) {
        btnRenewYes.onclick = async () => {
            if (pendingRenewActionTargetId !== null && pendingRenewActionMonthStr !== null) {
                const targetId = pendingRenewActionTargetId;
                const monthStr = pendingRenewActionMonthStr;
                ...
                renewListingAdmin(targetId, monthStr, true);
            }
        };
    }
