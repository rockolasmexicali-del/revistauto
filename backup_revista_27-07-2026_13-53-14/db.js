...
class Database {
    ...
    async syncWithServer() {
        try {
            const res = await fetch(`${this.apiBaseUrl}/listings?status=all`, { cache: 'no-store' });
            if (res.ok) {
                const data = await res.json();
                if (data.success && Array.isArray(data.listings)) {
                    this.isServerConnected = true;
                    // Actualizar caché local con la base de datos del servidor
                    localStorage.setItem(this.listingsKey, JSON.stringify(data.listings));
    ...
    getAllListings() {
        ...
        return listings.map(l => ({
            ...l,
            isMyListing: l.publisherId === this.uuid
        }));
    }
    ...
    saveListing(listing) {
        ...
        // Asincrónicamente enviar al servidor si está disponible
    ...
    deleteListing(id) {
        ...
        fetch(`${this.apiBaseUrl}/listings/${id}`, { method: 'DELETE' })
            .catch(err => console.log('Eliminado local (servidor offline)'));
    }
    ...
    isListingActive(listing) {
        ...
        if (!listing.lastRenewedMonth) return false;
    ...
    markAsSold(id) {
        ...
        fetch(`${this.apiBaseUrl}/listings/${id}/sold`, { method: 'PUT' })
            .catch(err => console.log('Vendido actualizado localmente'));
    }
    ...
    updateListing(id, updatedData) {
        ...
        fetch(`${this.apiBaseUrl}/listings/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(updatedData)
        }).catch(err => console.log('Actualizado localmente'));
    }
    ...
    addListingNote(id, noteText) {
        ...
        if (index !== -1) {
            ...
            fetch(`${this.apiBaseUrl}/listings/${id}/notes`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(newNote)
            }).catch(err => console.log('Nota guardada localmente'));
    ...
    addPayment(listingId, amount, receiptImage, type = 'Aprobación') {
        const listings = this.getAllListings();
        ...
        if (index !== -1) {
            ...
            fetch(`${this.apiBaseUrl}/listings/${listingId}/payments`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(newPayment)
            }).catch(err => console.log('Pago guardado localmente'));
            
            // Add CRM Note
            this.addListingNote(listingId, `Pago registrado: $${amount} MXN (${type})`);
